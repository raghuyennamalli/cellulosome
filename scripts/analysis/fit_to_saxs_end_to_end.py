...
distance_restraint_list_cellulosome = [
    {
        "prot1": "cipa", "residue1_start": 29,  "residue1_end": 29,
        "prot2": "cipa", "residue2_start": 1853, "residue2_end": 1853,
        "target_distance": 575.0, "threshold": 20.0,
        "label": "cellulosome end to end"
    }
]
...
for restraint in tqdm.tqdm(distance_restraint_list_cellulosome, desc='Calculating Global Dmax'):
...
ax_s.set_ylabel('Model Dmax - SAXS Dmax (Å)')
ax_s.set_title('End to end distance (Signed)', fontsize=14)
plt.tight_layout()
...
ax_a.set_title('Absolute Dmax Distribution', fontsize=14)
plt.tight_layout()
plt.savefig(f'{save_path}/saxs_end_to_end_validation.png', dpi=150)
plt.close(fig_abs)
...
write_and_print("-" * len(header)) # Adds a clean dividing line
...
results_file.close()
print('SAXS Validation completed!')
